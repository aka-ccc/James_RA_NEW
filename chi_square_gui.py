...
from chi_square_controller import exportChiSquareCsvFile
...
class analyzeGui:
    ...
    def exportChiSquare(self):
        if(self.filename==""):
            self.writeToMiniConsole('還沒有選擇欲計算特徵卡分值的csv檔\n')
            return
        outputFilePath=self.filename.replace('.csv','-特徵卡分值.csv')
        flag,msg=exportChiSquareCsvFile(inputFilePath=self.filename,outputFilePath=outputFilePath)
        if(flag): 
            self.writeToMiniConsole('特徵卡分值輸出至==>'+msg+'\n')
        else:
            self.writeToMiniConsole('出錯惹'+msg+'\n')
        self.miniConsole.config(state='disable')
    ...
    def __init__(self):
        ...
        #計算特徵卡分值
        exportUnclean=tk.Button(topFrame,text='計算特徵卡分值',fg='Green',command=self.exportChiSquare)
